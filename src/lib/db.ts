...
export async function saveMessage(
  ...
  try {
    const result = await client.query(
      'INSERT INTO conversation_history (role, content, pdf_id) VALUES ($1, $2, $3) RETURNING id, role, content, pdf_id, created_at;',
      [role, content, pdfId]
    );
...
export async function getConversationHistory(
  ...
  try {
    const query = pdfId
      ? 'SELECT id, role, content, pdf_id, created_at FROM conversation_history WHERE pdf_id = $1 ORDER BY created_at ASC LIMIT $2;'
      : 'SELECT id, role, content, pdf_id, created_at FROM conversation_history ORDER BY created_at ASC LIMIT $1;';
    
    const params = pdfId ? [pdfId, limit] : [limit];
